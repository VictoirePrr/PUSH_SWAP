...
/*   Updated: 2025/01/31 13:57:08 by vicperri         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
Then I go inside another loop to search for this min number and I make sure it
 was not already sorted (temp->index still to -1)
If it was not : then I set my index to data->index |
If it was : I coninue to traverse my temp until I reach the top.
I then increment data->index and I check if my stack still possess index = to -1
If it is : I go for antoher round |
If not : I stop. Every nodes have their index set !*/
...
If it finds an index = to -1 : my boolean changes to 0. |
If not : it continues until it reaches the head.
...
/* Goal : compute a size of chunk that is going to
grow exponentionaly with the size of the list
To do so :
Quadratic formula : ax² + bx + c = 0
With a, b and c being constants, and x being an unknown variable.
The first part helps with bigger lists :
The size of the chunk won't double between a list of 500,
and a list of 1000 but will be much bigger.
